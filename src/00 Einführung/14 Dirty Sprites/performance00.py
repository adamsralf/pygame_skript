    ...
    def update(self, *args: Any, **kwargs: Any) -> None:
        ...
        if "action" in kwargs.keys():
            if kwargs["action"] == "switch" and self._status == 0:
                self._timer_color = Timer(100, False)
                self.image.fill("red")
                self._status = 1
                self._counter -= 1
        if self._status == 1 and self._timer_color.is_next_stop_reached():
            self.image.fill("white")
            self._status = 0
...
class Game:
    def __init__(self) -> None:
        ...
        self._all_tiles = pygame.sprite.Group()
