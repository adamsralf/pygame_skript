...
class Moon:
    def __init__(self, screen: pygame.surface.Surface, layer_count: int=5):
        self.screen = screen
        top = Settings.WINDOW.height - Settings.HORIZONT
        self.rect = pygame.rect.Rect(0, top, 
                                      Settings.WINDOW.width, Settings.HORIZONT) # Landeplatz§\label{moonlander02a01}§

        self.layers = []
        for layer_index in range(layer_count): # Aufbau Gebirge§\label{moonlander02a02}§
            mycolor = 180 - layer_index * 20   # Vordergrund dunkler, Hintergrund heller§\label{moonlander02a03}§
            self.layers.append({"y":self.rect.top - 10 - randint(5, 30)*layer_index, 
    ...
    def draw(self):
        ...
        for layer in reversed(self.layers):
            ...
            r.height = self.rect.top - r.top
            pygame.draw.rect(self.screen, layer["color"], r)
