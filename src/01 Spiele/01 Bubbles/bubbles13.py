...
class SoundContainer:                                           # §\label{srcBubble1301}§
    def __init__(self) -> None:
        self._sounds = {}
        self._sounds["bubble"] = pygame.mixer.Sound(os.path.join(Settings.path["sound"], "plopp1.mp3"))
        self._sounds["burst"] = pygame.mixer.Sound(os.path.join(Settings.path["sound"], "burst.mp3"))
        self._sounds["clash"] = pygame.mixer.Sound(os.path.join(Settings.path["sound"], "glas.wav"))
...
class Game:
    def __init__(self) -> None:
        pygame.init()
        self._sounds = SoundContainer()                         # §\label{srcBubble1302}§
    ...
    def update(self) -> None:
        ...
        if not self._pausing and self._running:
            if self.check_bubblecollision():
                if not self._restarting:                        # §\label{srcBubble1303}§
                    self._sounds.get("clash").play()            # §\label{srcBubble1304}§
            ...
            if len(self._all_bubbles) <= Settings.max_bubbles:
                ...
                while tries > 0:
                    ...
                    if collided:
                        ...
                        self._sounds.get("bubble").play()       # §\label{srcBubble1305}§
                        break
    ...
    def sting(self, mousepos) -> None:
        for bubble in self._all_bubbles:
            if bubble.collidepoint(mousepos):
                self._sounds.get("burst").play()                # §\label{srcBubble1306}§
                bubble.stung()
