...
class Rock(pygame.sprite.Sprite):
    ...
    def update(self):
        if self.bumm:
            ...
            newpos = self.rect.move(self.direction_x, self.direction_y)
            if newpos.top <= 0 or newpos.bottom >= Settings.window['height']:
                self.direction_y *= -1
            if newpos.left <= 0 or newpos.right >= Settings.window['width']:
                self.direction_x *= -1
