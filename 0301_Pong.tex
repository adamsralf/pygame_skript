...
\begin{enumerate}
    ...
    \item Beendet wird mit der Taste \keys{\esc} oder per Mausklick auf rotes~„X“.\label{req0201StandardBeenden}
...
\lstsource{SRC/02 Beispiele/01 Pong/pong01.py}{1}{10}{python}{Pong (\reqref{req0201Standard}) -- Präambel und die Klasse \texttt{Settings}
}{srcPong01a}
...
\lstsource{SRC/02 Beispiele/01 Pong/pong01.py}{31}{64}{python}{Pong (\reqref{req0201Standard}) -- die Klasse \texttt{Game}
}{srcPong01c}

Der Vollständigkeit halber: 

\lstsource{SRC/02 Beispiele/01 Pong/pong01.py}{67}{999}{python}{Pong (\reqref{req0201Standard}) -- die Klasse \texttt{Game}
}{srcPong01d}
...
\begin{enumerate}
	...
	\item Der linke Schläger wird über die Taste \keys{w} nach oben und mit der Taste \keys{s} nach unten bewegt.\label{req0201SchlägerTastenLinks}
	\item Der rechte Schläger wird über die Taste \keys{\arrowkeyup} nach oben und mit der Taste \keys{\arrowkeydown} nach unten bewegt.\label{req0201SchlägerTastenRechts}
...
\lstsource{SRC/02 Beispiele/01 Pong/pong02.py}{31}{47}{python}{Pong (\reqref{req0201Schläger}) -- Der Konstruktor von \texttt{Paddle}
}{srcPong02a}

Die Methode \texttt{update()} verteilt die Aufgaben. Dabei wird bzgl. der Bewegung das Attribut \texttt{self.direction} entsprechend manipuliert (ab \zeiref{srcPong0205}). Soll der Schläger seine Position verändern, wird in \zeiref{srcPong0204} die Methode \texttt{move()} aufgerufen.

\lstsource{SRC/02 Beispiele/01 Pong/pong02.py}{49}{55}{python}{Pong (\reqref{req0201Schläger}) -- \texttt{Paddle.update()}
}{srcPong02b}

Verbleibt noch die Methode \texttt{move()}. Sie sieht komplizierter aus, als sie ist. Nachdem überprüft wurde, ob überhaupt irgendwas getan werden muss, wird in \zeiref{srcPong0207} die neue vertikale Position berechnet (die horizontale bleibt ja unverändert). Anschließend wird überprüft, ob der Schläger das Spielfeld verlassen hat. Falls \emph{Ja}, wird der Schläger an den oberen bzw. unteren Rand zurückversetzt.

\lstsource{SRC/02 Beispiele/01 Pong/pong02.py}{57}{63}{python}{Pong (\reqref{req0201Schläger}) -- \texttt{Paddle.move()}
}{srcPong02c}
...
\lstsource{SRC/02 Beispiele/01 Pong/pong02.py}{66}{76}{python}{Pong (\reqref{req0201Schläger}) -- Konstruktor von \texttt{Game}
}{srcPong02d}

In \texttt{update()} und \texttt{draw()} erfolgen lediglich der entsprechende Methodenaufruf der Spritegroup.

\lstsource{SRC/02 Beispiele/01 Pong/pong02.py}{90}{96}{python}{Pong (\reqref{req0201Schläger}) -- \texttt{Game.update()} und \texttt{Game.draw()}
}{srcPong02e}
...
\lstsource{SRC/02 Beispiele/01 Pong/pong02.py}{98}{117}{python}{Pong (\reqref{req0201Schläger}) -- \texttt{Game.watch\_for\_events()}
}{srcPong02f}
...
\lstsource{SRC/02 Beispiele/01 Pong/pong03.py}{8}{12}{python}{Pong (\reqref{req0201Ball}) -- \texttt{Settings}
}{srcPong03A}


Passend zu Requirement~\ref{req0201Ball}.\ref{req0201BallGröße} und~\ref{req0201Ball}.\ref{req0201BallGeschwindigkeit} werden in \zeiref{srcPong0301} und \zeiref{srcPong0302} die Größe und die Geschwindigkeit festgelegt. Der Start des Balls erfolgt häufiger und wird daher in die Methode \texttt{service()} (\zeiref{srcPong0303}) ausgelagert.

\lstsource{SRC/02 Beispiele/01 Pong/pong03.py}{68}{77}{python}{Pong (\reqref{req0201Ball}) -- Konstruktor von \texttt{Ball}
}{srcPong03a}

In \texttt{update()} werden die Aufgaben verteilt.

\lstsource{SRC/02 Beispiele/01 Pong/pong03.py}{79}{85}{python}{Pong (\reqref{req0201Ball}) -- \texttt{Ball.update()}
}{srcPong03b}

Schauen wir uns jetzt die Hilfsfunktionen im einzelnen an. Beginnen wir mit \texttt{move()}. Wie zu erwarten, werden die Positionsangabe mit Hilfe der Geschwindigkeiten aktualisiert. Danach wird ab \zeiref{srcPong0304} überprüft, ob der Ball eine der vier Ränder erreicht hat.

Wird der obere oder untere Rand erreicht (Requirement~\ref{req0201Ball}.\ref{req0201BallObenUnten}), wechselt das Vorzeichen der vertikalen Geschwindigkeit durch den Aufruf von \texttt{vertical\_flip()} (\srcref[vref]{srcPong03e}). Nach dem Flip wird der obere bzw. der untere Rand gesetzt, da es ja sein kann, dass der Ball die Randgrenze schon überschritten hat. 
...
\lstsource{SRC/02 Beispiele/01 Pong/pong03.py}{87}{100}{python}{Pong (\reqref{req0201Ball}) -- \texttt{Ball.move()}
}{srcPong03c}
...
\lstsource{SRC/02 Beispiele/01 Pong/pong03.py}{102}{105}{python}{Pong (\reqref{req0201Ball}) -- \texttt{Ball.service()}}{srcPong03d}

Der Richtungswechsel ist hier nur ein Vorzeichenwechsel. Die Methode \texttt{flip\_hori\-zon\-tal()} wird noch nicht verwendet, wird aber gebraucht, wenn wir den Ball vom Schläger abprallen lassen wollen. 

\lstsource{SRC/02 Beispiele/01 Pong/pong03.py}{107}{111}{python}{Pong (\reqref{req0201Ball}) -- Die Flip-Methoden von \texttt{Ball}}{srcPong03e}
...
\lstsource{SRC/02 Beispiele/01 Pong/pong04.py}{123}{131}{python}{Pong (\reqref{req0201Punkte}) -- Konstruktor von \texttt{Score}}{srcPong04a}

In dieser Methode wird der aktuelle Punktestand mit Hilfe des Font-Objektes gerendert und positioniert.

\lstsource{SRC/02 Beispiele/01 Pong/pong04.py}{139}{141}{python}{Pong (\reqref{req0201Punkte}) -- \texttt{Score.render()}}{srcPong04b}

In \texttt{update()} wird der passende Punktestand aktualisiert und \texttt{render()} aufgerufen. 

\newpage
\lstsource{SRC/02 Beispiele/01 Pong/pong04.py}{133}{137}{python}{Pong (\reqref{req0201Punkte}) -- \texttt{Score.update()}}{srcPong04c}
...
\lstsource{SRC/02 Beispiele/01 Pong/pong04.py}{14}{16}{python}{Pong (\reqref{req0201Punkte}) -- \texttt{MyEvent}}{srcPong04d}

Nun muss \texttt{Ball} nur noch das passende Event auslösen und \texttt{Game} das Event verwalten. Hier die Anpassungen in \texttt{Ball}. In der Methode \texttt{move()} werden die entsprechenden Stellen ersetzt. So wird beispielsweise in \zeiref{srcPong0402} die Nummer des Spielers in das Event gestopft, der den Punkt bekommt, und in \zeiref{srcPong0403} wird das Event abgeschickt.

\lstsource{SRC/02 Beispiele/01 Pong/pong04.py}{95}{110}{python}{Pong (\reqref{req0201Punkte}) -- \texttt{Ball.move()}}{srcPong04g}

Jetzt muss nur noch in \texttt{watch\_for\_events()} das Userevent abgegriffen werden (ab \zeiref{srcPong0404}).

\lstsource{SRC/02 Beispiele/01 Pong/pong04.py}{178}{199}{python}{Pong (\reqref{req0201Punkte}) -- \texttt{Ball.watch\_for\_events()}}{srcPong04h}
...
In \texttt{Game} bauen wir dazu die Methode \texttt{check\_collision()}, welche überprüft, ob der Ball einen Schläger getroffen hat. Es bietet sich an, dazu die Methode \texttt{ pygame\-.sprite\-.coll\-ide\_rect()}\myindex{pyg}{\texttt{sprite}!\texttt{collide\_rect()}} zu verwenden. Wenn eine Kollision vorliegt, wird die bisher noch nicht verwendete Methode \texttt{horizontal\_flip()} (siehe \srcref[vref]{srcPong03e}) über \texttt{update()} ausgeführt. Danach werden die Ränder wieder so verschoben, dass Ball und Schläger sich nicht überlappen. Ebenso wird über \texttt{update()} die Methode \texttt{respeed()} aufgerufen, so dass Requirement~\ref{req0201Schlag}.\ref{req0201SchlagGeschwindigkeit} erfüllt wird.

\lstsource{SRC/02 Beispiele/01 Pong/pong05.py}{207}{213}{python}{Pong (\reqref{req0201Schlag}) -- \texttt{Game.\_check\_collision()}}{srcPong05a}

In \texttt{respeed()} werden den Geschwindigkeitsvektoren jeweils um Zufallswerte erhöht. Über \texttt{speed} ist diese Schwankung indirekt von der Bildschirmgröße abhängig. 

\lstsource{SRC/02 Beispiele/01 Pong/pong05.py}{123}{125}{python}{Pong (\reqref{req0201Schlag}) -- \texttt{Ball.respeed()}}{srcPong05b}
...
\begin{enumerate}
	\item Über die Taste~\keys{1} wechselt die Steuerung vom linken Schläger zwischen Mensch und Computer.\label{req0201Computer1}

	\item Über die Taste~\keys{2} wechselt die Steuerung vom rechten Schläger zwischen Mensch und Computer.\label{req0201Computer2}
...
\lstsource{SRC/02 Beispiele/01 Pong/pong06.py}{8}{12}{python}{Pong (\reqref{req0201Computer}) -- \texttt{Settings}}{srcPong06a}
...
\lstsource{SRC/02 Beispiele/01 Pong/pong06.py}{194}{199}{python}{Pong (\reqref{req0201Computer}) -- \texttt{Game.update()}}{srcPong06b}
...
\lstsource{SRC/02 Beispiele/01 Pong/pong06.py}{251}{257}{python}{Pong (\reqref{req0201Computer}) -- \texttt{Game.paddlecontroler()}}{srcPong06c}
...
\newpage
\lstsource{SRC/02 Beispiele/01 Pong/pong06.py}{206}{241}{python}{Pong (\reqref{req0201Computer}) -- \texttt{Game.watch\_for\_events()}}{srcPong06d}
...
\begin{enumerate}
	...
	\item Der Sound soll über \keys{F2} ein- bzw. ausgeschaltet werden können.\label{req0201Sound3}
...
\newpage
\lstsource{SRC/02 Beispiele/01 Pong/pong07.py}{9}{21}{python}{Pong (\reqref{req0201Sound}) -- \texttt{Settings}}{srcPong07a}
...
\lstsource{SRC/02 Beispiele/01 Pong/pong07.py}{100}{114}{python}{Pong (\reqref{req0201Sound}) -- Konstruktor von \texttt{Ball}}{srcPong07b}


Den ersten Sound programmieren wir für das Abprallen am Schläger in \texttt{hori\-zontal\-\_flip()}. Nachdem abgefragt wurde, ob überhaupt eine Soundausgabe erfolgen soll, wird ermittelt, ob der Ball vom rechten oder vom linken Schläger abprallt. Dies geschieht indirekt durch die Abfrage, in welche Richtung der Ball aktuell fliegt (\zeiref{srcPong0703}). Passend dazu wird die Lautstärke so angepasst, dass der Eindruck entsteht, dass der Abprall links bzw. rechts vom Zuschauer erfolgt.

\lstsource{SRC/02 Beispiele/01 Pong/pong07.py}{149}{158}{python}{Pong (\reqref{req0201Sound}) -- \texttt{Ball.horizontal\_flip()}}{srcPong07d}

Etwas dynamischer wird dieser Soundeffekt in \texttt{vertical\_flip()} erzeugt. In \zeiref{srcPong0704} wird die relative horizontale Position ermittelt. Ist das Zentrum des Balls links, hat \texttt{rel\_pos} einen Wert nahe der~0; steht der Ball weit recht, hat er einen Wert nahe~1. Diese Werte können dann als rechte und linke Lautstärke in die Methode \texttt{set\_volume()} eingesetzt werden.

\lstsource{SRC/02 Beispiele/01 Pong/pong07.py}{160}{165}{python}{Pong (\reqref{req0201Sound}) -- \texttt{Ball.vertical\_flip()}}{srcPong07e}

Verbleibt noch das Ein- bzw. Ausschalten der Soundausgabe in \texttt{watch\_for\_events()} in \zeiref{srcPong0705} mit Hilfe der Funktionstaste \keys{F2}.

\lstsource{SRC/02 Beispiele/01 Pong/pong07.py}{232}{246}{python}{Pong (\reqref{req0201Sound}) -- \texttt{Ball.watch\_for\_events()}}{srcPong07f}
...
\begin{enumerate}
	\item Durch \keys{P} werden alle Aktivitäten gestoppt und das Spiel pausiert. Wird nochmal \keys{P} gedrückt, wird das Spiel fortgesetzt.\label{req0201PauseHelp1}
	
	\item Durch \keys{H} wird das Spiel pausiert und ein Hilfetext angezeigt. Wird nochmal \keys{H} gedrückt, wird das Spiel fortgesetzt.\label{req0201PauseHelp2}
\end{enumerate}
\er
...
\lstsource{SRC/02 Beispiele/01 Pong/pong08.py}{47}{52}{python}{Pong (\reqref{req0201PauseHelp}) -- \texttt{Pause}}{srcPong08a}
...
\lstsource{SRC/02 Beispiele/01 Pong/pong08.py}{55}{69}{python}{Pong (\reqref{req0201PauseHelp}) -- \texttt{Help}}{srcPong08b}
...
\lstsource{SRC/02 Beispiele/01 Pong/pong08.py}{218}{235}{python}{Pong (\reqref{req0201PauseHelp}) -- \texttt{Help}}{srcPong08c}
...
\lstsource{SRC/02 Beispiele/01 Pong/pong08.py}{249}{255}{python}{Pong (\reqref{req0201PauseHelp}) -- \texttt{Game.update()}}{srcPong08d}

In \texttt{draw()} wird ebenfalls auf die jeweiligen Modi abgefragt und ggf. das entsprechende Sprite ausgegeben.

\lstsource{SRC/02 Beispiele/01 Pong/pong08.py}{257}{264}{python}{Pong (\reqref{req0201PauseHelp}) -- \texttt{Game.draw()}}{srcPong08e}
