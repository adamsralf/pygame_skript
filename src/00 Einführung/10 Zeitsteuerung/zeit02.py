...
class Game(object):
    ...
    def new_bullet(self):
        if pygame.time.get_ticks() >= self.time_stamp + self.time_duration: # Wenn Intervallgrenze erreicht§\label{srcZeit0204}§ 
            b = Bullet("shoot.png")
            b.rect.centerx = self.enemy.sprite.rect.centerx
            b.rect.centery = self.enemy.sprite.rect.centery + 20
            self.all_bullets.add(b)
            self.time_stamp = pygame.time.get_ticks()                       # Neuer Intervallstart §\label{srcZeit0205}§ 
