import sys  # Wegen max int §\label{moonlander0601}§
...
class Lander:
    def __init__(self, screen: pygame.surface.Surface) -> None:
        ...
        self.fuel_initial = Settings.LEVEL["fair"]  # Starttreibstoff §\label{moonlander0603}§             
        self.fuel = self.fuel_initial             # Aktueller Treibstoff §\label{moonlander0604}§
        self.fuel_consumption = 20                  # Treibstoffverbrauch pro Sekunde §\label{moonlander0605}§
    ...
    def draw(self) -> None:
        if self.thrusting:
            ...
            self.screen.blit(self.surface, self.rect.topleft)
        print(self.fuel)       # Nur vorrübergehend§\label{moonlander0607}§

    def move(self) -> None:
        if self.thrusting and self.fuel > 0:                          # Treibstoff übrig?§\label{moonlander0608}§
            self.velocity += Settings.THRUST * Settings.DELTATIME
            self.fuel -= self.fuel_consumption * Settings.DELTATIME   # Treibstoffverbrauch§\label{moonlander0606}§
            if self.fuel < 0:
                self.thrusting = False
                self.fuel = 0
