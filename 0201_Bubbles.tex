...
\begin{enumerate}
    ...
    \item Alle Bitmaps werden als \texttt{pygame.sprite.Sprite}-Objekt erzeugt und nach dem Laden konvertiert und passend skaliert.\label{reqStandardSprite}\myindex{pyg}{\texttt{sprite}!{\texttt{Sprite}}}
    \item Alle Bitmaps -- außer dem Hintergrund -- sind transparent.\label{reqStandardTransparenz}
    \item Alle Bitmaps -- bis auf „Einzelkämpfer“ oder Hintergrund -- werden in \texttt{pygame.sprite.Group}-Objekten abgelegt.\label{reqStandardGruppe}\myindex{pyg}{\texttt{sprite}!{\texttt{Group}}}
    \item Das Spiel hat eine Ablaufgeschwindigkeit von $60~fps$.\label{reqStandardFps}
\end{enumerate}
\er


\reqref[vref]{reqStandard} regelt nicht nur Konkretes, sondern auch Allgemeines und wird deshalb bei späteren Implementierungen noch einmal auftauchen. Hier erst das, wass wir sofort umsetzten können.

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles01.py}{1}{19}{python}{Bubbles (\reqref{reqStandard}.\ref{reqStandardGröße}) -- Präambel und \texttt{Settings
}}{srcBubbles01a}
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles01.py}{22}{28}{python}{Bubbles (\reqref{reqStandard}.\ref{reqStandardHintergrund}) -- \texttt{Background
}}{srcBubbles01b}

Die Klasse \texttt{Background} ist ein einzelnes Sprite\index{self.image}, welches nur geladen und passend skaliert wird. Da sich der Hintergrund nicht ändert, muss kein \texttt{update()} implementiert werden. Auch wird keine Transparenz\index{Transparenz} benötigt. Dass wir eine eigene Kindklasse programmieren, ist ein wenig wie mit Pistolen auf Spatzen schießen. Wir hätten es auch als \texttt{Sprite}-Objekt implementieren können. Ich habe das hier nur der Übersichtlichkeit wegen gemacht. Das Hintergrundbild ist in \abbref[vref]{picAquarium01} zu sehen.

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles01.py}{30}{62}{python}{Bubbles (\reqref{reqStandard}) -- \texttt{Game}}{srcBubbles01c}  

In der Klasse \texttt{Game} werden in \texttt{\_\_init\_\_()} die Pygame üblichen Methoden \texttt{init()}\myindex{pyg}{\texttt{init()}}, \texttt{set\_mode()}\myindex{pyg}{\texttt{display}!\texttt{set\_mode()}}, \texttt{clock()}\myindex{pyg}{\texttt{time}!\texttt{Clock}} und \texttt{set\_caption()}\myindex{pyg}{\texttt{display}!\texttt{set\_caption()}} am Start aufgerufen. Auch wird der Hintergrund in ein \texttt{pygame.sprite.GroupSingle}-Objekt\myindex{pyg}{\texttt{sprite}!\texttt{GroupSingle}} abgelegt und das Flag der Hauptprogrammschleife erzeugt. Die Methoden \texttt{run()}, \texttt{watch\_for\_events()}, \texttt{update()} und \texttt{draw()} enthalten nur Basisfunktionalitäten, die hier nicht weiter erläutert werden müssen. Durch diese Methoden ist aber schon der generelle Ablauf des Spiels vorgegeben. Alle weiteren Eigenschaften des Spiels, sind nur noch Erweiterungen dieses Ablaufs, keine Veränderungen mehr. 

Zum Schluss erfogt nur noch noch der Aufruf (siehe \srcref[vref]{srcBubbles01s}).

\myebild{aquarium01}{0.20}{Bubbles: Hintergrundbild (aquarium.png)}{picAquarium01}

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles01.py}{65}{68}{python}{Bubbles (\reqref{reqStandard}) -- Aufruf}{srcBubbles01s}  
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles02.py}{17}{19}{python}{Bubbles (\reqref{reqBlasenErscheinen}) -- Ergänzungen in \texttt{Settings}}{srcBubbles02a}
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles02.py}{26}{38}{python}{Bubbles (\reqref{reqBlasenErscheinen}) -- \texttt{Timer}}{srcBubbles02b}  
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles02.py}{49}{64}{python}{Bubbles (\reqref{reqBlasenErscheinen}) -- \texttt{Bubble}}{srcBubbles02c}  

In der \zeiref{srcBubble0204} wird für den \texttt{Background} ein \texttt{GroupSingle}-Objekt\myindex{pyg}{\texttt{sprite}!\texttt{GroupSingle}} angelegt; das erspart mir ein wenig Tipparbeit, da ich \texttt{update()} und \texttt{draw()} über die üblichen SpriteGroup-Mechanismen aufrufen kann. \zeiref{srcBubble0205} erzeugt ein \texttt{Timer}-Objekt mit einer Intervalllänge von $500~ms$, wobei im ersten Intervall noch keine Blasen erzeugt werden sollen (siehe \reqref{reqBlasenErscheinen}.\ref{reqBlasenErscheinenIntervall}).

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles02.py}{67}{76}{python}{Bubbles (\reqref{reqBlasenErscheinen}) -- Konstruktor von \texttt{Game}}{srcBubbles02d}  
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles02.py}{86}{92}{python}{Bubbles (\reqref{reqBlasenErscheinen}) -- \texttt{draw()} und \texttt{update()} von \texttt{Game}}{srcBubbles02e}  

Die Grundidee hinter \texttt{spawn\_bubble()} ist, solange eine Position für eine neue Blase zu raten, bis man eine genügend freie Fläche gefunden hat. Damit man damit nicht in einer \Gls{endlosschleife} landet, wird die Anzahl der Versuche auf~100 begrenzt. Wird keine Freifläche gefunden, wird die Blase nicht der Spritegruppe hinzugefügt -- sie verfällt also. Der Radius wird dazu kurzfristig erweitert (\zeiref{srcBubble0207}) und nach der Kollisionsprüfung wieder auf seinen Ursprungswert reduziert (\zeiref{srcBubble0208}). 
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles02.py}{94}{107}{python}{Bubbles (\reqref{reqBlasenErscheinen}) -- \texttt{spawn\_bubble()} von \texttt{Game}}{srcBubbles02f}  
...
\myebild{aquarium02}{0.30}{Bubbles: Die Blasen haben beim Start einen Mindestabstand}{picAquarium02}
...
Die maximale Anzahl will ich in \texttt{Settings} festlegen. Ausgehend von der Fläche wird eine Obergrenze festgelegt:

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles03.py}{20}{20}{python}{Bubbles (\reqref{reqBlasenanzahl}) -- Ergänzung von \texttt{Settings}}{srcBubbles03a}  
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles03.py}{95}{109}{python}{Bubbles (\reqref{reqBlasenanzahl}) -- Ergänzung von \texttt{Game} in \texttt{spawn\_bubbles()}}{srcBubbles03b}  
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles04.py}{50}{61}{python}{Bubbles (\reqref{reqBlasenWachstum}.\ref{reqBlasenWachstumContainer}) -- \texttt{BubbleContainer}}{srcBubbles04a}  

Bisher wurde nur ein Startwert und damit eine untere Grenze für den Blasenradius in \texttt{Settings} definiert. Diese Definition wird nun in \zeiref{srcBubble0400} passend zu \reqref{reqBlasenWachstum}.\ref{reqBlasenWachstumMax} um die Angabe eines maximalen Radius erweitert.

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles04.py}{17}{17}{python}{Bubbles (\reqref{reqBlasenWachstum}.\ref{reqBlasenWachstumMax}) -- Erweiterung von \texttt{Settings}}{srcBubbles04b}  
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles04.py}{64}{77}{python}{Bubbles (\reqref{reqBlasenWachstum}) -- Ergänzung von \texttt{Bubble}}{srcBubbles04c}  
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles04.py}{111}{113}{python}{Bubbles (\reqref{reqBlasenWachstum}) -- Ergänzung von \texttt{update()} in \texttt{Game}}{srcBubbles04d}  
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles04.py}{118}{118}{python}{Bubbles (\reqref{reqBlasenWachstum}) -- Ergänzung von \texttt{spawn\_bubble()} in \texttt{Game}}{srcBubbles04e}  
...
\myebild{aquarium03}{0.20}{Bubbles: Die Blasen sind gewachsen/verwachsen}{picAquarium03}
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles05.py}{86}{89}{python}{Bubbles (\reqref{reqMauscursor}) -- \texttt{collidepoint()} in \texttt{Bubble}}{srcbubbles05a}  
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles05.py}{138}{148}{python}{Bubbles (\reqref{reqMauscursor}) -- \texttt{set\_mousecursor()} in \texttt{Game}}{srcbubbles05b}  
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles06.py}{104}{113}{python}{Bubbles (\reqref{reqBlasenzerplatzen}) -- \texttt{watch\_for\_event()} in \texttt{Game}}{srcbubbles06a}  
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles06.py}{153}{156}{python}{Bubbles (\reqref{reqBlasenzerplatzen}) -- \texttt{sting()} in \texttt{Game}}{srcbubbles06b}  
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles07.py}{23}{23}{python}{Bubbles (\reqref{reqPunktestand}.\ref{reqPunktestandNull}) -- Erweiterung von \texttt{Settings}}{srcbubbles07a}  
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles07.py}{93}{95}{python}{Bubbles (\reqref{reqPunktestand}.\ref{reqPunktestandRadius}) -- \texttt{stung()} in \texttt{Bubble}}{srcbubbles07b}  

Die Methode \texttt{sting()} in \texttt{Game} muss dazu passend verändert werden (\zeiref{srcBubble0704}).

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles07.py}{174}{177}{python}{Bubbles (\reqref{reqPunktestand}.\ref{reqPunktestandRadius}) -- \texttt{sting()} in \texttt{Game}}{srcbubbles07c}  
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles07.py}{22}{22}{python}{Bubbles (\reqref{reqPunktestand}.\ref{reqPunktestandAnzeige}) -- Erweiterung von \texttt{Settings}}{srcbubbles07d}  
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles07.py}{98}{107}{python}{Bubbles (\reqref{reqPunktestand}.\ref{reqPunktestandAnzeige}) -- \texttt{Points}}{srcbubbles07e}  

Verbleiben einige Erweiterungen in \texttt{Game}. Im Konstruktor wird das \texttt{Points}-Objekt in einer \texttt{GroupSingle}\myindex{pyg}{\texttt{sprite}!\texttt{GroupSingle}} verwaltet.

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles07.py}{117}{119}{python}{Bubbles (\reqref{reqPunktestand}.\ref{reqPunktestandAnzeige}) -- Erweiterung des Konstruktors von \texttt{Game}}{srcbubbles07f}  

Und in \texttt{draw()} und \texttt{update()} werden die entsprechenden Aufrufe eingefügt: siehe \zeiref{srcBubble0706} und \zeiref{srcBubble0705}.

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles07.py}{134}{144}{python}{Bubbles (\reqref{reqPunktestand}.\ref{reqPunktestandAnzeige}) -- Erweiterung von \texttt{update()}, \texttt{draw()} in \texttt{Game}}{srcbubbles07g}  
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles08.py}{185}{196}{python}{Bubbles (\reqref{reqSpielende}) -- \texttt{check\_bubblecollision()} in \texttt{Game}}{srcbubbles08b}  
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles08.py}{137}{139}{python}{Bubbles ()\reqref{reqSpielende}) -- Hilfslinien in \texttt{Game}}{srcbubbles08c}  
...
Für \reqref{reqZeitanpassungen}.\ref{reqZeitanpassungenWachsen} wird in \texttt{Bubble} ein \texttt{Timer}-Objekt benötigt.\index{Timer}\randnotiz{Timer} Dieses wird im Konstruktor mit den richtigen Werten in \zeiref{srcBubble0901} angelegt. In der Methode \texttt{update()} wird dann in \zeiref{srcBubble0902} abgefragt, ob die $100~ms$ schon verstrichen sind. Wenn \emph{Ja}, wird der Radius vergrößert; ansonsten passiert nichts.

\myindex{pyg}{\texttt{draw}!\texttt{rect()}}
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles09.py}{67}{82}{python}{Bubbles (\reqref{reqZeitanpassungen}.\ref{reqZeitanpassungenWachsen}) -- \texttt{Bubble}}{srcbubbles09a}  

Die andere Zeitanpassung wird in \texttt{Game} gesteuert. Auch dazu wird ein \texttt{Timer}-Objekt verwendet. Dieses wird im Konstruktor in \zeiref{srcBubble0905} erzeugt.

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles09.py}{121}{121}{python}{Bubbles (\reqref{reqZeitanpassungen}.\ref{reqZeitanpassungenErscheinen}) -- Konstruktor von \texttt{Game}}{srcbubbles09b}  

Für das Erstellen einer neuen Blase ist \texttt{spawn\_bubble()} zuständig. Deshalb muss auch dort die entsprechende Anpassung erfolgen. Alle $10~s$ wird die Bedingung in \zeiref{srcBubble0906} \true. Im Anweisungsblock der Verzweigung wird dann für den Timer, der das Erscheinen der Blasen taktet (\texttt{\_timer\_bubble}) die Intervalldauer um $100~ms$ reduziert. Die Maximumsbildung in der Zeile danach stellt sicher, dass die Intervalldauer mindestens $400~ms$ beträgt.

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles09.py}{155}{171}{python}{Bubbles (\reqref{reqZeitanpassungen}.\ref{reqZeitanpassungenErscheinen}) -- \texttt{spawn\_bubble()} von \texttt{Game}}{srcbubbles09c}  
...
Dazu braucht es einen zweiten \texttt{BubbleContainer} mit den skalierten roten Blasen. Um auf diese leichter zugreifen zu können, sind diese aus \texttt{Game} nach \texttt{Settings} verschoben worden. 
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles10.py}{24}{24}{python}{Bubbles (\reqref{reqKollisionanzeigen}) -- Erweiterung von \texttt{Settings}}{srcbubbles10a}  
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles10.py}{54}{59}{python}{Bubbles (\reqref{reqKollisionanzeigen}) -- Änderung Konstruktor von \texttt{BubbleContainer}}{srcbubbles10b}  

Der Konstruktor von \texttt{Game} füllt nun das Dictionary \texttt{bubble\_container} in \texttt{Settings} auf (\zeiref{srcBubble1003} und \zeiref{srcBubble1004}).

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles10.py}{117}{129}{python}{Bubbles (\reqref{reqKollisionanzeigen}) -- Änderung vom Konstruktor von \texttt{Game}}{srcbubbles10c}  

Durch das neue Attribut \texttt{mode} (\zeiref{srcBubble1005}) wird die Farbe der Blase bestimmt. Jedesmal, wenn nun das Image aus dem \texttt{BubbleContainer} geladen wird, wird über dieses Attribut gesteuert, welcher der beiden \texttt{BubbleContainer} als Datenquelle verwendet werden soll. Beispielhaft sei hier \zeiref{srcBubble1006} in \texttt{update()} erwähnt.

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles10.py}{68}{83}{python}{Bubbles (\reqref{reqKollisionanzeigen}) -- Konstruktor von \texttt{Bubble} und \texttt{update()}}{srcbubbles10d}  

Ändert sich der Modus, muss die andere Farbe nachgeladen werden. Dies erfüllt die Methode \texttt{set\_mode} in \texttt{Bubble}.

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles10.py}{85}{87}{python}{Bubbles (\reqref{reqKollisionanzeigen}) -- \texttt{set\_mode()} in \texttt{Bubble}}{srcbubbles10e}  

Jetzt muss nur noch im Falle einer Kollision -- also eines Spielendes -- der Modus geändert werden. In \abbref[vref]{picAquarium07} können Sie sehen, wie die beiden kollidierenden Blasen rot erscheinen. 

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles10.py}{196}{211}{python}{Bubbles (\reqref{reqKollisionanzeigen}) -- \texttt{check\_bubblecollision()} in \texttt{Game}}{srcbubbles10f}  

Damit mir Zeit bleibt, die Kollision zu sehen, will ich am Ende $2~s$ warten. Die Methode \texttt{pygame.time.wait()}\myindex{pyg}{\texttt{time}!\texttt{wait()}}\randnotiz{wait()} hält dei Anwendung entsprechend lang an (\zeiref{srcBubble1007}).

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles10.py}{213}{222}{python}{Bubbles (\reqref{reqKollisionanzeigen}) -- Wartezeit in \texttt{run()}}{srcbubbles10g}  
...
Zunächst werden die Pygame-Konstanten KEYUP\myindex{pyg}{\texttt{KEYUP}} und K\_p importiert (\zeiref{srcBubble1101}).

\myebild{aquarium08}{0.30}{Bubbles: Pausenbildschirm}{picAquarium08}

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles11.py}{7}{8}{python}{Bubbles (\reqref{reqPause}) -- Zusätzliche Konstanten}{srcbubbles11a}  
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles11.py}{148}{149}{python}{Bubbles (\reqref{reqPause}) -- \texttt{Konstruktor} in \texttt{Game}}{srcbubbles11b}  


In \texttt{watch\_for\_events()} wird nun abgefragt, ob die \texttt{P}-Taste (\zeiref{srcBubble1103}) oder die rechte Maustaste (\zeiref{srcBubble1104}) gedrückt wurde. Falls \emph{Ja}, wird der Boolesche-Wert des Flags negiert (\gls{toggling}).

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles11.py}{151}{165}{python}{Bubbles (\reqref{reqPause}) -- \texttt{watch\_for\_events()} in \texttt{Game}}{srcbubbles11c}  

Für die Darstellung der Pause, habe ich die -- vielleicht etwas überflüssige -- Klasse \texttt{Pause} implementiert. Im Konstruktor wird ein graues Image mit einer Halbtransparenz erzeugt (\zeiref{srcBubble1105}). Ebenso wird der große rote Schriftzug \emph{Pause} gerendert. Für beide sind \texttt{Image}- und \texttt{Rect}-Objekte vorhanden, so dass sie leicht über \texttt{draw()} ausgegeben werden können.

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles11.py}{54}{69}{python}{Bubbles (\reqref{reqPause}) -- \texttt{Pause}}{srcbubbles11d}  

Im Konstruktor von \texttt{Game} wird ein Objekt der Klasse \texttt{Pause} angelegt (siehe \srcref[vref]{srcbubbles11b}), damit es in \texttt{draw()} verwendet werden kann. In \zeiref{srcBubble1106} wird das Flag abgefragt; ggf. wird dann das halbtransparente Bitmaps und der Schiftzug über das Bild gemalt.

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles11.py}{167}{177}{python}{Bubbles (\reqref{reqPause}) -- \texttt{draw()} in \texttt{Game}}{srcbubbles11e}  

Aber wie wird nun verhindert, dass das Spiel während der Pause weiterläuft? Wenn wir uns in Erinnerung rufen, dass alle Zuständsänderungen von \texttt{update()} ausgehen, muss dort die Anpassung vorgenommen werden. Die Lösung ist dann recht schnell gefunden. In \zeiref{srcBubble1107} wird einfach abgefragt, ob das Spiel sich im Pausenmodus befindet. Falls \emph{Ja}, werden die Aufrufe der \texttt{update()}-Methoden der Sprites übersprungen -- das war's.

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles11.py}{179}{187}{python}{Bubbles (\reqref{reqPause}) -- \texttt{update()} in \texttt{Game}}{srcbubbles11f}  
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles12.py}{8}{9}{python}{Bubbles (\reqref{reqNeustart}) -- Erweiterung des Imports}{srcbubbles12a}  

Die Aufgabe, eine Rückfrage in den Vordergrund zu schieben, ist eigentlich schon mit der Klasse \texttt{Pause} gelöst; ich kann daher die Klasse verallgemeinern, indem ich sie in \texttt{Foreground} umbenenne und den Text dem Konstruktor als String-Parameter übergebe (\zeiref{srcBubble1202} und \zeiref{srcBubble1203}).

\myebild{aquarium09}{0.3}{Bubbles: Neustartbildschirm}{picAquarium09}

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles12.py}{56}{72}{python}{Bubbles (\reqref{reqNeustart}) -- von \texttt{Pause} zu \texttt{Foreground}}{srcbubbles12b}  
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles12.py}{150}{154}{python}{Bubbles (\reqref{reqNeustart}) -- Neue Attribute im Konstruktor von \texttt{Game}}{srcbubbles12c}  

Die Antwort muss in \texttt{watch\_for\_events()} abgefragt und in entsprechende Flaginhalte umgesetzt werden. Antwortet der Spieler mit~\emph{J} (\zeiref{srcBubble1205}), muss das Spiel ja neu gestartet werden. Deshalb wird \texttt{do\_start} auf \true\ gesetzt. Gibt er \emph{N} ein, soll das Spiel beendet werden, weshalb das Flag der Hauptprogrammschleife mit \false\ bestückt wird (\zeiref{srcBubble1206}).

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles12.py}{156}{174}{python}{Bubbles (\reqref{reqNeustart}) -- Erweiterung von \texttt{watch\_for\_events()}}{srcbubbles12d}  

In \texttt{update()} muss nun die Verarbeitung der Flags erfolgen. Gleich zu Beginn wird in \zeiref{srcBubble1208} abgefragt, ob der Spieler neu starten will -- also ein \emph{J} eingegeben hat. Der Punktestand wird zurückgesetzt; das Gleiche passiert mit den die Spielschwierigkeit steuernden \texttt{Timer}-Objekten. 

Danach werden die Flags wieder auf \false\ gesetzt, damit die Vordergrundanzeige verschwindet und das Spiel beim nächsten Frame nicht wieder einen Neustart durchführt.

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles12.py}{193}{208}{python}{Bubbles (\reqref{reqNeustart}) -- Erweiterung von \texttt{update()} in \texttt{Game}}{srcbubbles12e}  

In \texttt{draw()} wird nun ermittelt, ob die Vordergrundausgabe über das Spiel gelegt werden soll. Dazu wird in \zeiref{srcBubble1207} das Flag \texttt{\_restarting} abgefragt, um die entsprechende Methode in \texttt{Foreground} aufzurufen.

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles12.py}{176}{188}{python}{Bubbles (\reqref{reqNeustart}) -- Erweiterung von \texttt{draw()} in \texttt{Game}}{srcbubbles12f}  
...
\lstsource{SRC/01 Spiele/01 Bubbles/bubbles12.py}{262}{271}{python}{Bubbles (\reqref{reqNeustart}) -- \texttt{run()} in \texttt{Game}}{srcbubbles12g}  
...
Die Klasse für Soundeffekte ist \texttt{pygame.mixer.Sound}\myindex{pyg}{\texttt{mixer}!\texttt{Sound}}\randnotiz{Sound}. Dem Konstruktor wird dabei der Dateiname der Sound-Datei übergeben.

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles13.py}{72}{80}{python}{Bubbles (\reqref{reqSound}) -- \texttt{SoundContainer}}{srcbubbles13a}  

Im Konstruktor von \texttt{Game} wird das Container-Objekt definiert (\zeiref{srcBubble1302}). 

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles13.py}{145}{149}{python}{Bubbles (\reqref{reqSound}) -- \texttt{SoundContainer}}{srcbubbles13b}  


Nun müssen die Sounds nur noch an der geeigneten Stelle mit \texttt{pygame.mixer.Sound.\-play()}\myindex{pyg}{\texttt{mixer}!\texttt{Sound}!\texttt{play()}}\randnotiz{play()} abgespielt werden: in \texttt{spawn\_\-bubb\-le()} in \zeiref{srcBubble1305}.

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles13.py}{231}{236}{python}{Bubbles  (\reqref{reqSound}.\ref{reqSoundErscheinen}) -- \texttt{spawn\_bubble()}}{srcbubbles13c}  

und in \texttt{sting()} in \zeiref{srcBubble1306}:

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles13.py}{250}{254}{python}{Bubbles (\reqref{reqSound}.\ref{reqSoundZerstechen}) -- \texttt{sting()}}{srcbubbles13d}  

und in \texttt{update()}. Dabei muss noch berücksichtigt werden, ob das Spiel gerade den Vordergrund für den Neustart anzeigt. Wenn \emph{Ja} (\zeiref{srcBubble1303}), darf der Sound nicht noch einmal abgespielt werden; ansonsten wird permanent der Berühren-Sound abgespielt (\zeiref{srcBubble1304}). 

\lstsource{SRC/01 Spiele/01 Bubbles/bubbles13.py}{204}{221}{python}{Bubbles (\reqref{reqSound}.\ref{reqSoundBerühren}) -- \texttt{update()}}{srcbubbles13e}  

Und Schluss :-)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\newpage
%\subsection{Der Quelltext}

%Hier der redaktionell aufbereitete Gesamtquelltext.

%\lstsource{SRC/01 Spiele/01 Bubbles/bubbles99.py}{1}{999}{python}{Bubbles -- Endstand}{srcbubbles99}  

%\section{Wurst}

%\br{Blasenwachstum}{reqBlasenWachstumm}
%\begin{enumerate}
%	\item Die unterschiedlich großen Blasen werden in einem Kontainer zur Verfügung gestellt.
%	
%	\item Per Zufall bekommt jede Blase eine Wachstumsrate zwischen 1 und 4 Pixel pro Zeiteinheit.
%	
%	\item Die Zeiteinheit wird zufällig aus dem Intervall $[1000~ms, 5000~ms]$ bestimmt.
%\end{enumerate}
%
%\er
